...
- **Public (no auth):** `/health`, `/v1/ready`, `/v1/metrics`, **`/manual-verify`** (HTML tool), and **static UI assets** (`/`, `/index.html`, `/css/*`, `/icons/*`, `manifest`, `sw.js`, etc.) — see [src/config/public-routes.ts](../src/config/public-routes.ts).
