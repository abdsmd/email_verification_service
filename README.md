...
- **Manual verify UI** (pretty-printed full JSON): browse **`/manual-verify`** on the HTTP port — works even when **`PWA_ENABLED=false`** (implemented by [src/routes/manual-verify.route.ts](src/routes/manual-verify.route.ts) + [public/manual-verify.html](public/manual-verify.html)). Shared styling: [public/css/site.css](public/css/site.css).
